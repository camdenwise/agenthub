...
function StatCardIcon({ type, className }: { type: string; className: string }) {
  ...
  if (type === 'ai') {
    return (<svg className={base} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>)
  }
  if (type === 'leads') {
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (convos) {
        ...
        setNeedsHumanCount(convos.filter((c: any) => c.status === 'needs_human').length)
      ...
      if (leads) {
        ...
        setContactedLeads(leads.filter((l: any) => l.status === 'contacted').length)
        setConvertedLeads(leads.filter((l: any) => l.status === 'converted').length)
  ...
  const STAT_CARDS = [
    { title: 'Messages', value: String(totalConvos), subtitle: needsHumanCount > 0 ? `${needsHumanCount} need human response` : 'All handled by AI', href: '/messages', icon: 'messages', iconBg: 'bg-indigo-500/10', iconColor: 'text-indigo-700' },
    { title: 'AI handled', value: `${aiHandledPercent}%`, subtitle: `${aiHandledCount} of ${totalConvos} automated`, href: '/messages', icon: 'ai', iconBg: 'bg-emerald-500/10', iconColor: 'text-emerald-700' },
    { title: 'New leads', value: String(totalLeads), subtitle: `${contactedLeads} contacted, ${convertedLeads} converted`, href: '/leads', icon: 'leads', iconBg: 'bg-amber-500/10', iconColor: 'text-amber-800' },
    { title: 'Reviews sent', value: '0', subtitle: 'Review system coming soon', href: '/reviews', icon: 'reviews', iconBg: 'bg-pink-500/10', iconColor: 'text-pink-700' },
  ]
    ...
    <div className="flex flex-col">
      <div className="shrink-0">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {activeBusiness?.name ?? 'Your business'} – Today&apos;s overview
        </p>
      </div>
