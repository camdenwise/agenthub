...
function getAvatarColor(channel: Channel) {
  switch (channel) {
    case 'instagram': return 'bg-pink-500/15 text-pink-800'
    case 'facebook': return 'bg-indigo-500/15 text-indigo-800'
    case 'email': return 'bg-violet-500/15 text-violet-800'
    default: return 'bg-emerald-500/15 text-emerald-800'
  }
}
...
export default function MessagesPage() {
  ...
  // Load conversations when activeBusiness changes
  useEffect(() => {
    ...
    async function load() {
      ...
      channel = supabase
        .channel(`conversations-realtime-${activeBusiness.id}`)
        .on(
          'postgres_changes',
          {
            event: '*',
  ...
  async function handleSend() {
    if (!inputValue.trim() || sending || !selected || !activeBusiness) return
    ...
    try {
      ...
      if (result.isSpam) {
        setConversations((prev) =>
          prev.map((c) => c.id === selected.id ? { ...c, status: 'spam' as ConvoStatus } : c)
        )
      } else {
        ...
        setConversations((prev) =>
          prev.map((c) => c.id === selected.id ? { ...c, messages: finalMessages, status: newStatus as ConvoStatus, updated_at: new Date().toISOString() } : c)
        )
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Messages</h1>
        <p className="mt-1 text-sm text-slate-500">All message channels in one centralized inbox. Type as a customer to test live AI responses.</p>
      </div>
      ...
      <div className="mt-4 flex min-h-0 flex-1 gap-0 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
              ...
              return (
                <li key={conv.id}>
                  <button type="button" onClick={() => setSelectedId(conv.id)}
                    className={`flex w-full flex-col gap-0.5 px-4 py-3 text-left transition-colors ${isActive ? 'border-l-2 border-indigo-500 bg-indigo-500/5' : 'border-l-2 border-transparent hover:bg-slate-50/80'}`}>
                    <div className="flex items-start gap-3">
                      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-medium ${getAvatarColor(conv.channel)}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                          ...
                          {conv.status === 'needs_human' && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">Needs Human</span>}
                          {conv.status === 'resolved' && <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-800">Resolved</span>}
                          {conv.status === 'spam' && <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800">Spam</span>}
                        </div>
                      </div>
                    </div>
                  </button>
        ...
        {!selected ? (
          <div className="flex min-w-0 flex-1 items-center justify-center text-sm text-slate-400">Select a conversation</div>
        ) : (
          <div className="flex min-w-0 flex-1 flex-col bg-slate-50/50">
            <div className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 bg-white px-5 py-3">
              <div className="flex min-w-0 items-center gap-3">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-medium ${getAvatarColor(selected.channel)}`}>
            ...
            <div className="shrink-0 border-t border-slate-200 bg-white p-4">
              <div className="flex gap-3">
                <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                  ...
                  placeholder="Type as a customer to test AI..." disabled={sending}
                ...
                <button type="button" onClick={handleSend} disabled={sending || !inputValue.trim()}
