...
export default function SettingsPage() {
  ...
  const [webhookForm, setWebhookForm] = useState({ name: '', url: 'https://app.agenthub.io/api/webhooks/', type: 'inbound' as 'inbound' | 'outbound' })
  ...
  function saveChannel() {
    ...
    setChannels((prev) => [...prev, { id: `ch-${Date.now()}`, name: channelForm.name.trim(), description: channelForm.description.trim() || 'Add description', account: channelForm.account.trim() || undefined, connected: false, iconKey: 'generic' }])
    setModalOpen(null)
  }
  function removeChannel(id: string) { setChannels((prev) => prev.filter((c) => c.id !== id)); setRemoveConfirm(null) }

  function toggleApi(id: string) { setApis((prev) => prev.map((a) => (a.id === id ? { ...a, connected: !a.connected } : a))) }
  function updateApiField(id: string, field: keyof ApiItem, value: string) { setApis((prev) => prev.map((a) => (a.id === id ? { ...a, [field]: value } : a))) }
  function openApiModal() { setApiForm({ name: '', description: '', apiKey: '', webhookUrl: '', reviewLink: '' }); setModalOpen('api') }
  function saveApi() {
    if (!apiForm.name.trim()) return
    setApis((prev) => [...prev, { id: `api-${Date.now()}`, name: apiForm.name.trim(), description: apiForm.description.trim() || 'Configure connection', connected: false, iconKey: 'generic', apiKey: apiForm.apiKey.trim() || undefined, webhookUrl: apiForm.webhookUrl.trim() || undefined, reviewLink: apiForm.reviewLink.trim() || undefined }])
    setModalOpen(null)
  }
  function removeApi(id: string) { setApis((prev) => prev.filter((a) => a.id !== id)); setRemoveConfirm(null) }

  function updateWebhookUrl(id: string, url: string) { setWebhooks((prev) => prev.map((w) => (w.id === id ? { ...w, url } : w))) }
  function openWebhookModal() { setWebhookForm({ name: '', url: 'https://app.agenthub.io/api/webhooks/', type: 'inbound' }); setModalOpen('webhook') }
  function saveWebhook() {
    if (!webhookForm.name.trim() || !webhookForm.url.trim()) return
    setWebhooks((prev) => [...prev, { id: `wh-${Date.now()}`, name: webhookForm.name.trim(), url: webhookForm.url.trim(), type: webhookForm.type, active: false, lastReceived: 'Never', totalEvents: 0 }])
    setModalOpen(null)
  }
  function removeWebhook(id: string) { setWebhooks((prev) => prev.filter((w) => w.id !== id)); setRemoveConfirm(null) }
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:gap-8">
      <div className="shrink-0 lg:w-56">
        ...
        <p className="mt-1 text-sm text-slate-500">Configure your business, connections, and preferences.</p>
        <nav className="mt-6 space-y-0.5 rounded-2xl border border-slate-200/80 bg-white p-2 shadow-sm">
          {TABS.map((tab) => (
            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)}
              className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors ${activeTab === tab.id ? 'bg-indigo-500/10 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
      ...
      <div className="min-w-0 flex-1 pt-8 lg:pt-0 max-w-2xl">
              ...
              <div className="border-t border-slate-100 pt-4">
                ...
                <div className="mt-3 grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input type="text" value={profile.managerName} onChange={(e) => setProfile({ ...profile, managerName: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700">Manager Email</label>
                    <input type="email" value={profile.managerEmail} onChange={(e) => setProfile({ ...profile, managerEmail: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'channels' && (
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {channels.map((ch) => (
                <div key={ch.id} className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600"><ChannelIcon key_={ch.iconKey} /></div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-slate-900">{ch.name}</span>
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${ch.connected ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>{ch.connected ? 'Connected' : 'Disconnected'}</span>
                      </div>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <button type="button" onClick={() => toggleChannel(ch.id)} className={`rounded-lg px-4 py-2 text-sm font-medium ${ch.connected ? 'bg-slate-100 text-slate-700 hover:bg-slate-200' : 'bg-indigo-600 text-white hover:bg-indigo-500'}`}>{ch.connected ? 'Disconnect' : 'Connect'}</button>
                    <button type="button" onClick={() => setRemoveConfirm({ type: 'channel', id: ch.id, name: ch.name })} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-red-50 hover:border-red-200 hover:text-red-700">Remove</button>
                  </div>
                </div>
              ))}
            </div>
            <button type="button" onClick={openChannelModal} className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">+ Add new channel</button>
          </section>
        )}
                ...
                <div key={api.id} className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex gap-4 min-w-0 flex-1">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600"><ApiIcon key_={api.iconKey} /></div>
                      <div className="min-w-0 flex-1 space-y-3">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-medium text-slate-900">{api.name}</span>
                          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${api.connected ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>{api.connected && <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />}{api.connected ? 'Active' : 'Inactive'}</span>
                        </div>
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <button type="button" onClick={() => toggleApi(api.id)} className={`rounded-lg px-4 py-2 text-sm font-medium ${api.connected ? 'bg-slate-100 text-slate-700 hover:bg-slate-200' : 'bg-indigo-600 text-white hover:bg-indigo-500'}`}>{api.connected ? 'Disconnect' : 'Connect'}</button>
                      <button type="button" onClick={() => setRemoveConfirm({ type: 'api', id: api.id, name: api.name })} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-red-50 hover:border-red-200 hover:text-red-700">Remove</button>
                    </div>
                  </div>
                </div>
              ))}
            </div>
            <button type="button" onClick={openApiModal} className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">+ Add new API</button>
        ...
        {activeTab === 'webhooks' && (
          <section className="space-y-4">
                ...
                <div key={wh.id} className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button type="button" onClick={() => copyToClipboard(wh.url, 'URL')} className="shrink-0 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Copy URL</button>
                    </div>
                    <div className="flex justify-end">
                      <button type="button" onClick={() => setRemoveConfirm({ type: 'webhook', id: wh.id, name: wh.name })} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-red-50 hover:border-red-200 hover:text-red-700">Remove</button>
                    </div>
                  </div>
            ...
            <button type="button" onClick={openWebhookModal} className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">+ Add new webhook</button>
            ...
            <div className="mt-6 border-t border-slate-100 pt-6">
              ...
              <div className="mt-3 flex gap-3">
                <button type="button" onClick={() => setNotifChannel('email')}
                  ...
                  <div><p className="font-medium text-slate-900">Email</p><p className="text-sm text-slate-500">{profile.managerEmail || 'Not set'}</p></div>
                </button>
                <button type="button" onClick={() => setNotifChannel('sms')}
                  ...
                  <span className="text-2xl">📱</span>
                  <div><p className="font-medium text-slate-900">SMS</p><p className="text-sm text-slate-500">{profile.phone || 'Not set'}</p></div>
                </button>
              </div>
            </div>
          </section>
        ...
        {activeTab === 'billing' && (
          <section className="space-y-6">
            <div><h2 className="text-lg font-semibold text-slate-900">Billing</h2><p className="mt-1 text-sm text-slate-500">Manage your subscription and payment details.</p></div>
            ...
            <div className="max-w-md rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
              ...
              <button type="button" className="mt-4 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-500">Add payment method</button>
            </div>
            <div className="max-w-md rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
      ...
      {modalOpen === 'channel' && (
        <Modal title="Add channel" onClose={() => setModalOpen(null)}>
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-slate-700">Name</label><input type="text" value={channelForm.name} onChange={(e) => setChannelForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Instagram DMs" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">Description</label><input type="text" value={channelForm.description} onChange={(e) => setChannelForm((f) => ({ ...f, description: e.target.value }))} placeholder="What this channel does" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">Account (optional)</label><input type="text" value={channelForm.account} onChange={(e) => setChannelForm((f) => ({ ...f, account: e.target.value }))} placeholder="e.g. @handle or email" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900" /></div>
      ...
      {modalOpen === 'api' && (
        <Modal title="Add API connection" onClose={() => setModalOpen(null)}>
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-slate-700">Name</label><input type="text" value={apiForm.name} onChange={(e) => setApiForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Meta Graph API" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">Description</label><input type="text" value={apiForm.description} onChange={(e) => setApiForm((f) => ({ ...f, description: e.target.value }))} placeholder="What this API does" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">API Key (optional)</label><input type="text" value={apiForm.apiKey} onChange={(e) => setApiForm((f) => ({ ...f, apiKey: e.target.value }))} placeholder="Paste API key" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 font-mono text-sm text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">Webhook URL (optional)</label><input type="text" value={apiForm.webhookUrl} onChange={(e) => setApiForm((f) => ({ ...f, webhookUrl: e.target.value }))} placeholder="https://..." className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 font-mono text-sm text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">Review link (optional)</label><input type="text" value={apiForm.reviewLink} onChange={(e) => setApiForm((f) => ({ ...f, reviewLink: e.target.value }))} placeholder="https://..." className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 font-mono text-sm text-slate-900" /></div>
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setModalOpen(null)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
      ...
      {removeConfirm && (
        <Modal title="Remove connection" onClose={() => setRemoveConfirm(null)}>
          <p className="text-slate-600">Are you sure you want to remove &quot;{removeConfirm.name}&quot;?</p>
          <div className="mt-6 flex justify-end gap-2">
            <button type="button" onClick={() => setRemoveConfirm(null)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
            <button type="button" onClick={confirmRemove} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-500">Remove</button>
          </div>
        </Modal>
      )}
      {modalOpen === 'webhook' && (
        <Modal title="Add webhook" onClose={() => setModalOpen(null)}>
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-slate-700">Name</label><input type="text" value={webhookForm.name} onChange={(e) => setWebhookForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Website Contact Form" className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">URL</label><input type="text" value={webhookForm.url} onChange={(e) => setWebhookForm((f) => ({ ...f, url: e.target.value }))} placeholder="https://app.agenthub.io/api/webhooks/..." className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 font-mono text-sm text-slate-900" /></div>
            <div><label className="block text-sm font-medium text-slate-700">Type</label><select value={webhookForm.type} onChange={(e) => setWebhookForm((f) => ({ ...f, type: e.target.value as 'inbound' | 'outbound' }))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-slate-900"><option value="inbound">Inbound</option><option value="outbound">Outbound</option></select></div>
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setModalOpen(null)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
...
function UserIcon() { return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" /></svg> }
function ChatIcon() { return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" /></svg> }
function LinkIcon() { return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244" /></svg> }
function WebhookIcon() { return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg> }
function CardIcon() { return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" /></svg> }
